package net.itskittyyoutube.kitty.groups;

import net.fabricmc.fabric.api.itemgroup.v1.FabricItemGroup;
import net.itskittyyoutube.kitty.KittysMod;
import net.itskittyyoutube.kitty.block.ModBlocks;
import net.itskittyyoutube.kitty.item.ModItems;
import net.minecraft.item.ItemGroup;
import net.minecraft.item.ItemStack;
import net.minecraft.item.Items;
import net.minecraft.registry.Registries;
import net.minecraft.registry.Registry;
import net.minecraft.text.Text;
import net.minecraft.util.Identifier;

public class ModGroups {
    public static final ItemGroup MUSIC_DISCS_GROUP = Registry.register(Registries.ITEM_GROUP,
            Identifier.of(KittysMod.MOD_ID, "music_disc_items"),
            FabricItemGroup.builder().icon(() -> new ItemStack(Items.MUSIC_DISC_13))
                    .displayName(Text.translatable("itemgroup.kitty.music_disc_items"))
                    .entries((displayContext, entries) -> {
                        entries.add(Items.MUSIC_DISC_13);
                        entries.add(Items.MUSIC_DISC_CAT);
                        entries.add(Items.MUSIC_DISC_BLOCKS);
                        entries.add(Items.MUSIC_DISC_CHIRP);
                        entries.add(Items.MUSIC_DISC_FAR);
                        entries.add(Items.MUSIC_DISC_MALL);
                        entries.add(Items.MUSIC_DISC_MELLOHI);
                        entries.add(Items.MUSIC_DISC_STAL);
                        entries.add(Items.MUSIC_DISC_STRAD);
                        entries.add(Items.MUSIC_DISC_WARD);
                        entries.add(Items.MUSIC_DISC_11);
                        entries.add(Items.MUSIC_DISC_CREATOR_MUSIC_BOX);
                        entries.add(Items.MUSIC_DISC_WAIT);
                        entries.add(Items.MUSIC_DISC_CREATOR);
                        entries.add(Items.MUSIC_DISC_PRECIPICE);
                        entries.add(Items.MUSIC_DISC_OTHERSIDE);
                        entries.add(Items.MUSIC_DISC_RELIC);
                        entries.add(Items.MUSIC_DISC_5);
                        entries.add(Items.MUSIC_DISC_PIGSTEP);
                        entries.add(Items.MUSIC_DISC_TEARS);
                        entries.add(Items.MUSIC_DISC_LAVA_CHICKEN);
                        entries.add(ModItems.MUSIC_DISC_THE_BYE_JUSTIN_HI_JUSTIN_RAP);
                        entries.add(ModItems.MUSIC_DISC_THE_TALE_OF_TOOTHLESS);
                        entries.add(ModItems.MUSIC_DISC_STEVE_LAVA_CHICKEN);
                        entries.add(ModItems.MUSIC_DISC_LAVA_TEARSSTEP);

                    }).build());

    public static final ItemGroup WOOD_GROUP = Registry.register(Registries.ITEM_GROUP,
            Identifier.of(KittysMod.MOD_ID, "wood"),
            FabricItemGroup.builder().icon(() -> new ItemStack(Items.OAK_PLANKS))
                    .displayName(Text.translatable("itemgroup.kitty.wood"))
                    .entries((displayContext, entries) -> {
                        entries.add(Items.OAK_LOG);
                        entries.add(Items.OAK_WOOD);
                        entries.add(Items.STRIPPED_OAK_LOG);
                        entries.add(Items.STRIPPED_OAK_WOOD);
                        entries.add(Items.OAK_PLANKS);
                        entries.add(Items.OAK_STAIRS);
                        entries.add(Items.OAK_SLAB);
                        entries.add(Items.OAK_FENCE);
                        entries.add(Items.OAK_FENCE_GATE);
                        entries.add(Items.OAK_DOOR);
                        entries.add(Items.OAK_TRAPDOOR);
                        entries.add(Items.OAK_PRESSURE_PLATE);
                        entries.add(Items.OAK_BUTTON);
                        entries.add(Items.SPRUCE_LOG);
                        entries.add(Items.SPRUCE_WOOD);
                        entries.add(Items.STRIPPED_SPRUCE_LOG);
                        entries.add(Items.STRIPPED_SPRUCE_WOOD);
                        entries.add(Items.SPRUCE_PLANKS);
                        entries.add(Items.SPRUCE_STAIRS);
                        entries.add(Items.SPRUCE_SLAB);
                        entries.add(Items.SPRUCE_FENCE);
                        entries.add(Items.SPRUCE_FENCE_GATE);
                        entries.add(Items.SPRUCE_DOOR);
                        entries.add(Items.SPRUCE_TRAPDOOR);
                        entries.add(Items.SPRUCE_PRESSURE_PLATE);
                        entries.add(Items.SPRUCE_BUTTON);
                        entries.add(Items.BIRCH_LOG);
                        entries.add(Items.BIRCH_WOOD);
                        entries.add(Items.STRIPPED_BIRCH_LOG);
                        entries.add(Items.STRIPPED_BIRCH_WOOD);
                        entries.add(Items.BIRCH_PLANKS);
                        entries.add(Items.BIRCH_STAIRS);
                        entries.add(Items.BIRCH_SLAB);
                        entries.add(Items.BIRCH_FENCE);
                        entries.add(Items.BIRCH_FENCE_GATE);
                        entries.add(Items.BIRCH_DOOR);
                        entries.add(Items.BIRCH_TRAPDOOR);
                        entries.add(Items.BIRCH_PRESSURE_PLATE);
                        entries.add(Items.BIRCH_BUTTON);
                        entries.add(Items.JUNGLE_LOG);
                        entries.add(Items.JUNGLE_WOOD);
                        entries.add(Items.STRIPPED_JUNGLE_LOG);
                        entries.add(Items.STRIPPED_JUNGLE_WOOD);
                        entries.add(Items.JUNGLE_PLANKS);
                        entries.add(Items.JUNGLE_STAIRS);
                        entries.add(Items.JUNGLE_SLAB);
                        entries.add(Items.JUNGLE_FENCE);
                        entries.add(Items.JUNGLE_FENCE_GATE);
                        entries.add(Items.JUNGLE_DOOR);
                        entries.add(Items.JUNGLE_TRAPDOOR);
                        entries.add(Items.JUNGLE_PRESSURE_PLATE);
                        entries.add(Items.JUNGLE_BUTTON);
                        entries.add(Items.ACACIA_LOG);
                        entries.add(Items.ACACIA_WOOD);
                        entries.add(Items.STRIPPED_ACACIA_LOG);
                        entries.add(Items.STRIPPED_ACACIA_WOOD);
                        entries.add(Items.ACACIA_PLANKS);
                        entries.add(Items.ACACIA_STAIRS);
                        entries.add(Items.ACACIA_SLAB);
                        entries.add(Items.ACACIA_FENCE);
                        entries.add(Items.ACACIA_FENCE_GATE);
                        entries.add(Items.ACACIA_DOOR);
                        entries.add(Items.ACACIA_TRAPDOOR);
                        entries.add(Items.ACACIA_PRESSURE_PLATE);
                        entries.add(Items.ACACIA_BUTTON);
                        entries.add(Items.DARK_OAK_LOG);
                        entries.add(Items.DARK_OAK_WOOD);
                        entries.add(Items.STRIPPED_DARK_OAK_LOG);
                        entries.add(Items.STRIPPED_DARK_OAK_WOOD);
                        entries.add(Items.DARK_OAK_PLANKS);
                        entries.add(Items.DARK_OAK_STAIRS);
                        entries.add(Items.DARK_OAK_SLAB);
                        entries.add(Items.DARK_OAK_FENCE);
                        entries.add(Items.DARK_OAK_FENCE_GATE);
                        entries.add(Items.DARK_OAK_DOOR);
                        entries.add(ModBlocks.DARK_OAK_DOOR_WITH_WINDOWS);
                        entries.add(Items.DARK_OAK_TRAPDOOR);
                        entries.add(ModBlocks.DARK_OAK_TRAPDOOR_WITH_WINDOWS);
                        entries.add(Items.DARK_OAK_PRESSURE_PLATE);
                        entries.add(Items.DARK_OAK_BUTTON);
                        entries.add(Items.MANGROVE_LOG);
                        entries.add(Items.MANGROVE_WOOD);
                        entries.add(Items.STRIPPED_MANGROVE_LOG);
                        entries.add(Items.STRIPPED_MANGROVE_WOOD);
                        entries.add(Items.MANGROVE_PLANKS);
                        entries.add(Items.MANGROVE_STAIRS);
                        entries.add(Items.MANGROVE_SLAB);
                        entries.add(Items.MANGROVE_FENCE);
                        entries.add(Items.MANGROVE_FENCE_GATE);
                        entries.add(Items.MANGROVE_DOOR);
                        entries.add(Items.MANGROVE_TRAPDOOR);
                        entries.add(Items.MANGROVE_PRESSURE_PLATE);
                        entries.add(Items.MANGROVE_BUTTON);
                        entries.add(Items.CHERRY_LOG);
                        entries.add(Items.CHERRY_WOOD);
                        entries.add(Items.STRIPPED_CHERRY_LOG);
                        entries.add(Items.STRIPPED_CHERRY_WOOD);
                        entries.add(Items.CHERRY_PLANKS);
                        entries.add(Items.CHERRY_STAIRS);
                        entries.add(Items.CHERRY_SLAB);
                        entries.add(Items.CHERRY_FENCE);
                        entries.add(Items.CHERRY_FENCE_GATE);
                        entries.add(Items.CHERRY_DOOR);
                        entries.add(Items.CHERRY_TRAPDOOR);
                        entries.add(Items.CHERRY_PRESSURE_PLATE);
                        entries.add(Items.CHERRY_BUTTON);
                        entries.add(Items.PALE_OAK_LOG);
                        entries.add(Items.PALE_OAK_WOOD);
                        entries.add(Items.STRIPPED_PALE_OAK_LOG);
                        entries.add(Items.STRIPPED_PALE_OAK_WOOD);
                        entries.add(Items.PALE_OAK_PLANKS);
                        entries.add(Items.PALE_OAK_STAIRS);
                        entries.add(Items.PALE_OAK_SLAB);
                        entries.add(Items.PALE_OAK_FENCE);
                        entries.add(Items.PALE_OAK_FENCE_GATE);
                        entries.add(Items.PALE_OAK_DOOR);
                        entries.add(Items.PALE_OAK_TRAPDOOR);
                        entries.add(Items.PALE_OAK_PRESSURE_PLATE);
                        entries.add(Items.PALE_OAK_BUTTON);
                        entries.add(Items.BAMBOO_BLOCK);
                        entries.add(Items.STRIPPED_BAMBOO_BLOCK);
                        entries.add(Items.BAMBOO_PLANKS);
                        entries.add(Items.BAMBOO_MOSAIC);
                        entries.add(Items.BAMBOO_STAIRS);
                        entries.add(Items.BAMBOO_MOSAIC_STAIRS);
                        entries.add(Items.BAMBOO_SLAB);
                        entries.add(Items.BAMBOO_MOSAIC_SLAB);
                        entries.add(Items.BAMBOO_FENCE);
                        entries.add(Items.BAMBOO_FENCE_GATE);
                        entries.add(Items.BAMBOO_DOOR);
                        entries.add(Items.BAMBOO_TRAPDOOR);
                        entries.add(Items.BAMBOO_PRESSURE_PLATE);
                        entries.add(Items.BAMBOO_BUTTON);
                        entries.add(Items.CRIMSON_STEM);
                        entries.add(Items.CRIMSON_HYPHAE);
                        entries.add(Items.STRIPPED_CRIMSON_STEM);
                        entries.add(Items.STRIPPED_CRIMSON_HYPHAE);
                        entries.add(Items.CRIMSON_PLANKS);
                        entries.add(Items.CRIMSON_STAIRS);
                        entries.add(Items.CRIMSON_SLAB);
                        entries.add(Items.CRIMSON_FENCE);
                        entries.add(Items.CRIMSON_FENCE_GATE);
                        entries.add(Items.CRIMSON_DOOR);
                        entries.add(Items.CRIMSON_TRAPDOOR);
                        entries.add(Items.CRIMSON_PRESSURE_PLATE);
                        entries.add(Items.CRIMSON_BUTTON);
                        entries.add(Items.WARPED_STEM);
                        entries.add(Items.WARPED_HYPHAE);
                        entries.add(Items.STRIPPED_WARPED_STEM);
                        entries.add(Items.STRIPPED_WARPED_HYPHAE);
                        entries.add(Items.WARPED_PLANKS);
                        entries.add(Items.WARPED_STAIRS);
                        entries.add(Items.WARPED_SLAB);
                        entries.add(Items.WARPED_FENCE);
                        entries.add(Items.WARPED_FENCE_GATE);
                        entries.add(Items.WARPED_DOOR);
                        entries.add(Items.WARPED_TRAPDOOR);
                        entries.add(Items.WARPED_PRESSURE_PLATE);
                        entries.add(Items.WARPED_BUTTON);
                        entries.add(ModBlocks.PLANKS_TEMPLATE);
                        entries.add(ModBlocks.WHITE_PLANKS);
                        entries.add(ModBlocks.WHITE_STAIRS);
                        entries.add(ModBlocks.WHITE_SLAB);
                        entries.add(ModBlocks.WHITE_FENCE);
                        entries.add(ModBlocks.WHITE_FENCE_GATE);
                        entries.add(ModBlocks.WHITE_DOOR);
                        entries.add(ModBlocks.WHITE_TRAPDOOR);
                        entries.add(ModBlocks.WHITE_PRESSURE_PLATE);
                        entries.add(ModBlocks.WHITE_BUTTON);
                        entries.add(ModBlocks.LIGHT_GRAY_PLANKS);
                        entries.add(ModBlocks.LIGHT_GRAY_STAIRS);
                        entries.add(ModBlocks.LIGHT_GRAY_SLAB);
                        entries.add(ModBlocks.LIGHT_GRAY_FENCE);
                        entries.add(ModBlocks.LIGHT_GRAY_FENCE_GATE);
                        entries.add(ModBlocks.LIGHT_GRAY_DOOR);
                        entries.add(ModBlocks.LIGHT_GRAY_TRAPDOOR);
                        entries.add(ModBlocks.LIGHT_GRAY_PRESSURE_PLATE);
                        entries.add(ModBlocks.LIGHT_GRAY_BUTTON);
                        entries.add(ModBlocks.GRAY_PLANKS);
                        entries.add(ModBlocks.GRAY_STAIRS);
                        entries.add(ModBlocks.GRAY_SLAB);
                        entries.add(ModBlocks.GRAY_FENCE);
                        entries.add(ModBlocks.GRAY_FENCE_GATE);
                        entries.add(ModBlocks.GRAY_DOOR);
                        entries.add(ModBlocks.GRAY_TRAPDOOR);
                        entries.add(ModBlocks.GRAY_PRESSURE_PLATE);
                        entries.add(ModBlocks.GRAY_BUTTON);
                        entries.add(ModBlocks.BLACK_PLANKS);
                        entries.add(ModBlocks.BLACK_STAIRS);
                        entries.add(ModBlocks.BLACK_SLAB);
                        entries.add(ModBlocks.BLACK_FENCE);
                        entries.add(ModBlocks.BLACK_FENCE_GATE);
                        entries.add(ModBlocks.BLACK_DOOR);
                        entries.add(ModBlocks.BLACK_TRAPDOOR);
                        entries.add(ModBlocks.BLACK_PRESSURE_PLATE);
                        entries.add(ModBlocks.BLACK_BUTTON);
                        entries.add(ModBlocks.BROWN_PLANKS);
                        entries.add(ModBlocks.BROWN_STAIRS);
                        entries.add(ModBlocks.BROWN_SLAB);
                        entries.add(ModBlocks.BROWN_FENCE);
                        entries.add(ModBlocks.BROWN_FENCE_GATE);
                        entries.add(ModBlocks.BROWN_DOOR);
                        entries.add(ModBlocks.BROWN_TRAPDOOR);
                        entries.add(ModBlocks.BROWN_PRESSURE_PLATE);
                        entries.add(ModBlocks.BROWN_BUTTON);
                        entries.add(ModBlocks.RED_PLANKS);
                        entries.add(ModBlocks.RED_STAIRS);
                        entries.add(ModBlocks.RED_SLAB);
                        entries.add(ModBlocks.RED_FENCE);
                        entries.add(ModBlocks.RED_FENCE_GATE);
                        entries.add(ModBlocks.RED_DOOR);
                        entries.add(ModBlocks.RED_TRAPDOOR);
                        entries.add(ModBlocks.RED_PRESSURE_PLATE);
                        entries.add(ModBlocks.RED_BUTTON);
                        entries.add(ModBlocks.ORANGE_PLANKS);
                        entries.add(ModBlocks.ORANGE_STAIRS);
                        entries.add(ModBlocks.ORANGE_SLAB);
                        entries.add(ModBlocks.ORANGE_FENCE);
                        entries.add(ModBlocks.ORANGE_FENCE_GATE);
                        entries.add(ModBlocks.ORANGE_DOOR);
                        entries.add(ModBlocks.ORANGE_TRAPDOOR);
                        entries.add(ModBlocks.ORANGE_PRESSURE_PLATE);
                        entries.add(ModBlocks.ORANGE_BUTTON);
                        entries.add(ModBlocks.YELLOW_PLANKS);
                        entries.add(ModBlocks.YELLOW_STAIRS);
                        entries.add(ModBlocks.YELLOW_SLAB);
                        entries.add(ModBlocks.YELLOW_FENCE);
                        entries.add(ModBlocks.YELLOW_FENCE_GATE);
                        entries.add(ModBlocks.YELLOW_DOOR);
                        entries.add(ModBlocks.YELLOW_TRAPDOOR);
                        entries.add(ModBlocks.YELLOW_PRESSURE_PLATE);
                        entries.add(ModBlocks.YELLOW_BUTTON);
                        entries.add(ModBlocks.LIME_PLANKS);
                        entries.add(ModBlocks.LIME_STAIRS);
                        entries.add(ModBlocks.LIME_SLAB);
                        entries.add(ModBlocks.LIME_FENCE);
                        entries.add(ModBlocks.LIME_FENCE_GATE);
                        entries.add(ModBlocks.LIME_DOOR);
                        entries.add(ModBlocks.LIME_TRAPDOOR);
                        entries.add(ModBlocks.LIME_PRESSURE_PLATE);
                        entries.add(ModBlocks.LIME_BUTTON);
                        entries.add(ModBlocks.GREEN_PLANKS);
                        entries.add(ModBlocks.GREEN_STAIRS);
                        entries.add(ModBlocks.GREEN_SLAB);
                        entries.add(ModBlocks.GREEN_FENCE);
                        entries.add(ModBlocks.GREEN_FENCE_GATE);
                        entries.add(ModBlocks.GREEN_DOOR);
                        entries.add(ModBlocks.GREEN_TRAPDOOR);
                        entries.add(ModBlocks.GREEN_PRESSURE_PLATE);
                        entries.add(ModBlocks.GREEN_BUTTON);
                        entries.add(ModBlocks.CYAN_PLANKS);
                        entries.add(ModBlocks.CYAN_STAIRS);
                        entries.add(ModBlocks.CYAN_SLAB);
                        entries.add(ModBlocks.CYAN_FENCE);
                        entries.add(ModBlocks.CYAN_FENCE_GATE);
                        entries.add(ModBlocks.CYAN_DOOR);
                        entries.add(ModBlocks.CYAN_TRAPDOOR);
                        entries.add(ModBlocks.CYAN_PRESSURE_PLATE);
                        entries.add(ModBlocks.CYAN_BUTTON);
                        entries.add(ModBlocks.LIGHT_BLUE_PLANKS);
                        entries.add(ModBlocks.LIGHT_BLUE_STAIRS);
                        entries.add(ModBlocks.LIGHT_BLUE_SLAB);
                        entries.add(ModBlocks.LIGHT_BLUE_FENCE);
                        entries.add(ModBlocks.LIGHT_BLUE_FENCE_GATE);
                        entries.add(ModBlocks.LIGHT_BLUE_DOOR);
                        entries.add(ModBlocks.LIGHT_BLUE_TRAPDOOR);
                        entries.add(ModBlocks.LIGHT_BLUE_PRESSURE_PLATE);
                        entries.add(ModBlocks.LIGHT_BLUE_BUTTON);
                        entries.add(ModBlocks.BLUE_PLANKS);
                        entries.add(ModBlocks.BLUE_STAIRS);
                        entries.add(ModBlocks.BLUE_SLAB);
                        entries.add(ModBlocks.BLUE_FENCE);
                        entries.add(ModBlocks.BLUE_FENCE_GATE);
                        entries.add(ModBlocks.BLUE_DOOR);
                        entries.add(ModBlocks.BLUE_TRAPDOOR);
                        entries.add(ModBlocks.BLUE_PRESSURE_PLATE);
                        entries.add(ModBlocks.BLUE_BUTTON);
                        entries.add(ModBlocks.PURPLE_PLANKS);
                        entries.add(ModBlocks.PURPLE_STAIRS);
                        entries.add(ModBlocks.PURPLE_SLAB);
                        entries.add(ModBlocks.PURPLE_FENCE);
                        entries.add(ModBlocks.PURPLE_FENCE_GATE);
                        entries.add(ModBlocks.PURPLE_DOOR);
                        entries.add(ModBlocks.PURPLE_TRAPDOOR);
                        entries.add(ModBlocks.PURPLE_PRESSURE_PLATE);
                        entries.add(ModBlocks.PURPLE_BUTTON);
                        entries.add(ModBlocks.MAGENTA_PLANKS);
                        entries.add(ModBlocks.MAGENTA_STAIRS);
                        entries.add(ModBlocks.MAGENTA_SLAB);
                        entries.add(ModBlocks.MAGENTA_FENCE);
                        entries.add(ModBlocks.MAGENTA_FENCE_GATE);
                        entries.add(ModBlocks.MAGENTA_DOOR);
                        entries.add(ModBlocks.MAGENTA_TRAPDOOR);
                        entries.add(ModBlocks.MAGENTA_PRESSURE_PLATE);
                        entries.add(ModBlocks.MAGENTA_BUTTON);
                        entries.add(ModBlocks.PINK_PLANKS);
                        entries.add(ModBlocks.PINK_STAIRS);
                        entries.add(ModBlocks.PINK_SLAB);
                        entries.add(ModBlocks.PINK_FENCE);
                        entries.add(ModBlocks.PINK_FENCE_GATE);
                        entries.add(ModBlocks.PINK_DOOR);
                        entries.add(ModBlocks.PINK_TRAPDOOR);
                        entries.add(ModBlocks.PINK_PRESSURE_PLATE);
                        entries.add(ModBlocks.PINK_BUTTON);

                    }).build());


    public static void registerGroups() {
        KittysMod.LOGGER.info("Registering Groups for " + KittysMod.MOD_ID);
    }
}